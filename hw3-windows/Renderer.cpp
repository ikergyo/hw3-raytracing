...
void Renderer::SceneRendering(std::string filename) {
	...
	m_Scene = FileReader::readfile(filename.c_str());
...
;
	...
	if (current_depth <= m_Scene->maxdepth) {
		...
		if (!(spec.x == 0.0f && spec.y == 0.0f && spec.z == 0.0f)) {
			...
			//vec3 reflectionDirection = normalize(2.0f * (dot(eyedir, intData.normal) * intData.normal) - eyedir);
...
//return true if the ray is blocked with an object
